...
`email` e CRP são normalizados antes das restrições de unicidade. O CRP é
...
- `201`: `{ token, user, psychologistId, crpStatus, onboardingStatus, termsVersion }`.
...
O `token` é destinado ao BFF, que deve colocá-lo em cookie `HttpOnly`; clientes
